...
function Login() {
  ...
  useEffect(() => {
    ...
    const token = localStorage.getItem("relawan_iqbaldinda_token");
    if (token) {
  ...
  useEffect(() => {
    async function fetchSanityData() {
      try {
        const sanityData = await sanityClient.fetch(`*[_type == 'user-relawan-iqbaldinda']{
          _id,
          name,
          email,
          type,
  ...
  const handleLogin = (event) => {
    ...
    const user = serverData.data.find(
      (relawan_iqbaldinda_userData) => relawan_iqbaldinda_userData.email === username && relawan_iqbaldinda_userData.password === password
    );
    ...
    if (user) {
      ...
      localStorage.setItem("relawan_iqbaldinda_token", token);
      localStorage.setItem("relawan_iqbaldinda_user", user.name);
      localStorage.setItem("relawan_iqbaldinda_id", user._id);
      localStorage.setItem('relawan_iqbaldinda_userData', JSON.stringify(filteredData));
